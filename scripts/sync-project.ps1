...
# Source: project entry point in the repo (see .opencode/conventions.md for the canonical path)
...
if (-not (Test-Path -LiteralPath $repoProject)) {
    throw "Repo project file not found: $repoProject (see .opencode/conventions.md for the expected path)"
}
...
# Master source: mirror of the project entry point + review metadata
$sourceContent = @"
# Project Context (Master Source)

> **ROLE**: master source file (mirrors repo's project entry point; see `.opencode/conventions.md`)
> **SOURCE REPO**: `$repoRoot/<project_entry_point>` (see `.opencode/conventions.md`)
> **LAST REVIEWED**: $today
...
> Edit the project entry point (see `.opencode/conventions.md`) in the repo and re-run the sync.
...
# This is NOT a copy of the project entry point. It is the project's slang/lunfardo dictionary.
