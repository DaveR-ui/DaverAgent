...
- Each `project.md` here is the source for session snapshots.
    - Prefer copying from the repo's `docs/project.md` and adding review metadata.
...
> **SOURCE REPO**: `$RepoPath/docs/project.md`
> **LAST REVIEWED**: $today
...
> Edit `docs/project.md` in the repo and re-run bootstrap with `-RefreshDocumentation`.
...
"@
    ...
    $projectSourceContent = Expand-Template -TemplatePath $projectSourceTemplate -Values @{
        ...
        ARCHITECTURE = "- TODO"
        COMMANDS = "- TODO"
        NOTES = "- Repo docs/project.md not found during bootstrap"
    }
    Write-FileIfNeeded -Path $projectSource -Content $projectSourceContent
}
