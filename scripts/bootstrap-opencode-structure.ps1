...
- Prefer copying from the repo's `.opencode/project.md` and adding review metadata.
...
$repoProjectMd = Join-Path $RepoPath ".opencode\project.md"
...
> **SOURCE REPO**: `$RepoPath/.opencode/project.md`
> **LAST REVIEWED**: $today
...
"@
    ...
    $projectSourceContent = Expand-Template -TemplatePath $projectSourceTemplate -Values @{
        ...
        ARCHITECTURE = "- TODO"
        COMMANDS = "- TODO"
        NOTES = "- Repo .opencode/project.md not found during bootstrap"
    }
    Write-FileIfNeeded -Path $projectSource -Content $projectSourceContent
}
