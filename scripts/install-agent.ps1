...
function Get-AgentBody {
    param([string]$Id)
    $coderBody = "# Coder Subagent`n`nImplement features, fix bugs, refactor code.`n`n**Project context**: read `docs/project.md` (entry point) for the tech stack, and `docs/context/README.md` to find relevant context docs.`n`n## Rules`n`n- Read `docs/context/README.md` to find the relevant context docs for your task`n- Follow the architecture doc in `docs/context/` for layering`n- Follow the rules/standards doc in `docs/context/` for development standards`n- All comments and docs in ENGLISH`n- Never commit without explicit instruction"
    $testerBody = "# Tester Subagent`n`nWrite and run tests.`n`n**Project context**: read `docs/project.md` (entry point). For test conventions see the rules/standards doc in `docs/context/`.`n`n## Rules`n`n- Tests next to source files`n- Mock external deps`n- All test names and comments in ENGLISH"
    $reviewerBody = "# Reviewer Subagent`n`nAnalyze code - never modify it.`n`n**Project context**: read `docs/project.md` (entry point) and `docs/context/README.md` to find relevant context docs.`n`n## Checklist`n`n1. Architecture compliance (see architecture doc in `docs/context/`)`n2. Development standards (see rules/standards doc in `docs/context/`)`n3. Security`n4. Performance`n5. Anti-patterns`n6. Testing`n`n## Output`n`nSee `.opencode/docs/agent-output-protocol.md` for the full format."
    $architectBody = "# Architect Subagent`n`nDesign system architecture, define module boundaries, establish patterns.`n`n**Project context**: read `docs/project.md` (entry point) and `docs/context/README.md` to find architecture and business-logic docs.`n`n## Principles`n`n- Read `docs/context/` for the project's architecture patterns and follow them`n- Favor simplicity`n- Design for testability and maintainability`n- Document decisions with rationale`n- All documentation in ENGLISH"
    $explorerBody = "# Explorer Subagent`n`nRead and analyze the codebase - never modify code.`n`n**Project context**: read `docs/project.md` (entry point) for module layout, then drill into the relevant source paths.`n`n## Approach`n`n- Use grep/glob/read effectively`n- Report file paths and line numbers`n- For architectural questions, consult the architecture doc in `docs/context/`"
    $documenterBody = "# Documenter Subagent`n`nWrite and maintain documentation.`n`n**Project context**: read `docs/project.md` and `docs/context/README.md` to find relevant context docs.`n`n## Rules`n`n- One topic per file`n- Reference, do not duplicate`n- All documentation in ENGLISH"
