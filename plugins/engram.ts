...
const ENGRAM_BIN = process.env.ENGRAM_BIN ?? Bun.which("engram") ?? "/home/admin/.local/bin/engram"
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
// NOTE: upstream's extractProjectName() helper was removed in this machine-local
// copy — the project bucket is fixed to "global" in the plugin export below, so
// no cwd-derived project name is computed anymore.
...
export const Engram: Plugin = async (ctx) => {
  // ── Machine-local divergence from upstream `engram setup opencode` output ──
  // Automatic writes are pinned to the shared "global" project so they land in
  // the same bucket as the MCP server (`--project=global`) and the manual
  // `mem_*` tools. Upstream derives the project from the session cwd via
  // extractProjectName() and renames it on startup; with a fixed project there
  // is nothing to migrate. Re-running `engram setup opencode` overwrites this
  // file and restores the upstream cwd-derived behavior (which, on engram v2,
  // can also fail closed when the cwd is ambiguous).
  //
  // If the MCP bucket in opencode.json ever changes, change this literal too.
  ...
  // NOTE: upstream's one-time `/projects/migrate` rename is intentionally not
  // performed here. The bucket is fixed to "global", so there is no cwd-derived
  // old name to migrate — and rewriting existing per-project memories into
  // "global" would be an unrequested, destructive consolidation.
