...
All documentation in `.agents/` follows these principles for optimal AI consumption:
...
### 1. For Context Documentation (`context/`)
Every markdown file in `.agents/context/` MUST start with:
...
```
> [!NOTE]
> `tags` are used for internal lookup via `_TAG-INDEX.md` and are NOT interpreted by OpenCode agents as skill metadata.

### 2. For OpenCode Skills (`skills/*/SKILL.md`)
Every `SKILL.md` MUST follow the [OpenCode Official Standard](https://opencode.ai/docs/es/skills/#_top):

```yaml
---
name: internal-skill-name
description: specific trigger phrase for the agent (1-1024 chars)
license: MIT
compatibility: opencode
metadata:
  last_updated: YYYY-MM-DD
...
```
> [!IMPORTANT]
> `tags` are NOT valid for agents. Use `metadata` for internal tracking. The `name` must match the directory name and follow the regex `^[a-z0-9]+(-[a-z0-9]+)*$`.
...
Every Skill (`SKILL.md`) MUST explicitly reference its authoritative context documentation.
- Use a "Reference" or "Context" section.
- Skills should provide the logic, while documentation provides the standards and examples.
- **Example**: A skill for creating components should link to `context/standards/coding-conventions.md`.
...
After creating a context doc, add a row to `.agents/context/orchestration/AGENTS.md`:
