...
It generates `docs/project.md`, the selected `docs/context/*.md` stubs, the
`docs/context/context-index.md` hub, the generated `docs/tag-index.md`, and the
project slang snapshot, then installs the derived validator at `docs/validate.js`
and runs it (Node required). It does **not** create agents or a project
`opencode.json` — agents are global.

Validate a project's docs at any time with:

```bash
node docs/validate.js            # report errors/warnings (exit 1 on errors)
node docs/validate.js --write    # regenerate the tag index (write-if-diff)
```

To migrate a project created before the `documentation-onrails` adoption (legacy
`docs/context/README.md`, `docs/protocols/README.md`, `docs/_TAG-INDEX.md`,
4-column Slices), follow the checklist in
