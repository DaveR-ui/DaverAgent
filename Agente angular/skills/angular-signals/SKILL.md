...
// Always provide a debug name for Angular DevTools
const selectedUser = signal<User | null>(null, { debugName: 'selectedUser' });
```
...
const hasItems = computed(() => items().length > 0, { debugName: 'hasItems' });
```
...
- All `signal()`, `computed()`, and `linkedSignal()` declarations should include a `debugName`.
- Use the property name as the `debugName` value unless there is a strong reason to expose a different label.
- Treat missing `debugName` on new signals as a standards violation, not as an optional cleanup.
