...
> **Project standard:** prefer `rxResource` for new reactive HTTP data loading. `httpResource` examples in this file are legacy compatibility references unless otherwise noted.
...
export class UserService {
  ...
  currentUser = rxResource({
    params: () => {
      const id = this.currentUserId();
      return id ? { id } : undefined;
    },
    stream: ({ params }) => this.http.get<User>(`${this.baseUrl}/${params.id}`)
  });
...
export class UsersListComponent {
  ...
  usersResource = rxResource({
    ...
    stream: ({ params }) => this.http.get<PaginatedResponse<User>>('/api/users', {
      params: {
