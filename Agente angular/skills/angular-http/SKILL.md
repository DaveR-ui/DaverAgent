---
name: angular-http
description: Implement HTTP data fetching in Angular v20+ using rxResource as the project standard, plus HttpClient for advanced transport scenarios and legacy httpResource maintenance.
---

# Angular HTTP & Data Fetching

Fetch data in Angular using `rxResource` as the project standard, plus traditional `HttpClient` for advanced transport scenarios.

> **Project standard:** prefer `rxResource` for new reactive HTTP data loading. Treat `httpResource` as legacy compatibility guidance only.

## rxResource() - Project Standard

Use `rxResource()` for new reactive HTTP flows because it aligns with the rest of the repository guidance and supports RxJS composition directly:
...
export class UserProfileComponent {
  ...
  userResource = rxResource({
    ...
    stream: ({ params }) => this.http.get<User>(`/api/users/${params.id}`).pipe(map((user) => user))
  });
}
```
...
Keep `httpResource()` only when maintaining existing code that already uses it correctly. Do not introduce it as the default for new component data flows.
