...
> **CRITICAL**: For Jasmine/Karma specific setup, teardown, and cleanup recipes, see the [Jasmine Test Standard](../jasmine-test-standard/SKILL.md). All new Jasmine spec files must start with `/// <reference types="jasmine" />`.
...
> Prefer `rxResource` for new tests. The example below is retained as legacy `httpResource` maintenance guidance.
