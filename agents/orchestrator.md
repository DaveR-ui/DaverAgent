...
- `coder` - implementation, bug fixes, refactoring (returns `CoderOutput`)
- `tester` - tests, coverage, e2e (returns `TesterOutput`)
- `reviewer` - code review, security, performance (returns `ReviewerOutput`)
- `architect` - system design, patterns (returns `ArchitectOutput`)
- `explorer` - codebase exploration, read-only (returns `ExplorerOutput`)
